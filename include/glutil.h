...
public:
	Shader(std::string vertexFileName = "shader.vert",
		std::string fragmentFileName = "shader.frag",std::string shadersPath = "bin",
	...
	void setF32(const i8* name, f32 a) const {
		glUniform1f(glGetUniformLocation(pid, name), a);
		//glUniform1
	}
	
	// Texture loading
	ui32 loadTexture(const std::string& textureFile,unsigned int &texture,
		               i32 param=GL_LINEAR) {
		//ui32 texture;
